...
const Quizzes = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-8 pb-8">
      ...
      {Quizzes.map((item, k) => (
        <div className="w-5/6">
          <QuizCard qid={item.id} quizNo={item.quizNo} title={item.title} attempts={item.attempts} total={item.total} />
        </div>
      ))}
    </div>
  );
...
const QuizCard = ({ qid, quizNo, title, attempts, total }) => {
      ...
      <CardContent>
        <div class="flex justify-between items-start ">
          ...
          <div className="flex flex-col gap-2 justify-end items-end">
            ...
            {menu && <VisibilityMenu />}
            <button className="text-custom-red font-bold text-[1.5rem]" onClick={()=>navigate(`/course/${id}/quiz/${qid}`)}>View Quiz</button>
            <button className="text-custom-red font-bold text-[1.5rem]" onClick={()=>navigate(`/course/${id}/quizSummary/${qid}`)}>View Submissions</button>
          </div>
        </div>
      </CardContent>
    </MUICard>
